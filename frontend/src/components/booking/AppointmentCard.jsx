function AppointmentCard({ appointment, onMarkComplete, isCounsellor = false }) {
  return (
    <div className="card bg-base-100 border border-base-300 shadow-sm hover:shadow-lg transition-all duration-300 rounded-lg sm:rounded-xl">
      <div className="card-body py-2.5 sm:py-3 px-3 sm:px-4">
        <h2 className="font-bold text-sm sm:text-base lg:text-lg break-words text-base-content tracking-tight">
          {isCounsellor ? appointment.studentName : appointment.counsellorName} ({appointment.academicYear})
        </h2>

        <p className="text-base-content/70 flex items-start gap-1.5 sm:gap-2 text-xs sm:text-sm lg:text-base mt-1 break-words">
