...
function Landing() {
  ...
  useEffect(() => {
    ...
    const counterTimer = setInterval(() => {
      ...
      const nextValues = targets.map((target) => Math.max(1, Math.round(target * progress)));
    ...
    <div className="min-h-screen bg-base-100 animate-fadeIn">
      ...
      <section className="relative min-h-[106vh] flex items-center overflow-hidden px-4 py-32">
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full">
          <div className="max-w-4xl">
            ...
            <p className="text-lg sm:text-2xl text-slate-100/90 leading-relaxed mb-8 max-w-3xl animate-slideUp" style={{ animationDelay: "0.1s" }}>
              {user
            ...
            <div className="max-w-3xl rounded-2xl bg-white/10 backdrop-blur-md border border-white/30 px-6 py-4 mb-10 animate-slideUp" style={{ animationDelay: "0.15s" }}>
              <p className="text-slate-100 text-base sm:text-lg italic transition-all duration-700">
                "{quotes[quoteIndex]}"
              </p>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 mb-14 animate-slideUp" style={{ animationDelay: "0.2s" }}>
              {!user ? (
                <>
                  <Link
                    to="/login"
                    className="btn btn-lg bg-sky-100 hover:bg-white text-sky-900 border-0 font-bold shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
                  >
                  ...
                  <Link
                    to="/register"
                    className="btn btn-outline btn-lg font-bold text-white border-white/70 hover:bg-white hover:text-sky-900 transition-all duration-300"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex flex-wrap gap-8 mb-14 text-sm animate-slideUp" style={{ animationDelay: "0.3s" }}>
      ...
      <div id="offerings" className="bg-[#f5f2ed] py-20 px-4 relative">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-6 overflow-x-auto pb-2 lg:overflow-visible">
            {features.map((feature, index) => (
              ...
              >
                <div className={`h-36 bg-gradient-to-r ${feature.topBg} relative`}>
                  ...
                  <div className="relative h-full px-6 flex items-center gap-3 text-white">
                    <feature.Icon size={50} strokeWidth={2.2} />
                    <h3 className="text-3xl text-center font-bold leading-tight">{feature.title}</h3>
                  </div>
                </div>

                <div className="px-6 py-7">
                  <p className="text-[1.06rem] text-[#243047] leading-8 text-center">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="bg-[#f5f2ed] py-8 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-center text-base md:text-lg text-slate-800 max-w-4xl mx-auto leading-relaxed mb-10">
            Explore the spectrum of care from proactive wellness support to guided intervention, treatment, and long-term maintenance.
          </p>
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 text-white text-center">
            {statsData.map((item, index) => {
              ...
              return (
                <div key={item.label}>
                  <p className="text-5xl md:text-6xl font-extrabold italic tracking-tight mb-4">
                    {displayValue}
                  </p>
